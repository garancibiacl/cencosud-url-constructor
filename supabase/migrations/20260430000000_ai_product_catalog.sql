...
-- ── 1. product_catalog ───────────────────────────────────────
-- Cache de datos de producto por SKU. TTL manejado en app (24h).
-- Un mismo SKU puede existir por brand, por eso la PK es (sku, brand).
...
CREATE TABLE public.product_catalog (
  ...
  discount      NUMERIC(5,2),       -- porcentaje (0-100)
  image_url     TEXT,               -- imagen catálogo (fallback)
  description   TEXT,
  attributes    JSONB       NOT NULL DEFAULT '{}'::jsonb,   -- peso, volumen, etc.
  raw_payload   JSONB       NOT NULL DEFAULT '{}'::jsonb,   -- respuesta original de la API
  fetched_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT product_catalog_sku_brand_unique UNIQUE (sku, brand)
);

-- Búsqueda por SKU es el caso de uso principal
CREATE INDEX idx_product_catalog_sku_brand
  ON public.product_catalog (sku, brand);

-- Para limpiar registros expirados con una consulta eficiente
CREATE INDEX idx_product_catalog_fetched_at
  ON public.product_catalog (fetched_at DESC);

-- RLS: cualquier usuario autenticado puede leer y escribir
-- (la validación de que sea un SKU válido ocurre en la Edge Function)
...
-- Tracking de cada generación de imagen. Permite auditoría,
-- rate limiting y reintento en caso de fallo.
...
CREATE TABLE public.ai_generation_jobs (
  ...
  prompt        TEXT,                        -- prompt final enviado a fal.ai
  error_message TEXT,
  fal_request_id TEXT,                       -- ID de request en fal.ai (trazabilidad)
  duration_ms   INTEGER,                    -- tiempo total de generación
  created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
  completed_at  TIMESTAMPTZ
);

-- Rate limiting: contar generaciones por user en ventana de 1h
CREATE INDEX idx_ai_generation_jobs_user_created
  ON public.ai_generation_jobs (user_id, created_at DESC);

-- Búsqueda por campaña para coherencia visual (seed)
CREATE INDEX idx_ai_generation_jobs_campaign
...
-- ── 3. ai_images ────────────────────────────────────────────
-- Imágenes generadas. Una generación puede tener N variantes
-- (fal.ai puede devolver múltiples imágenes en una llamada).

CREATE TABLE public.ai_images (
  id            UUID        NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  job_id        UUID        NOT NULL REFERENCES public.ai_generation_jobs(id) ON DELETE CASCADE,
  user_id       UUID        NOT NULL,
  storage_path  TEXT        NOT NULL,        -- path dentro del bucket 'ai-images'
  public_url    TEXT        NOT NULL,
  width         INTEGER,
  height        INTEGER,
  file_size_bytes INTEGER,
  prompt        TEXT,
  seed          BIGINT,                      -- seed usada — reproduce el resultado
  model         TEXT,                        -- 'flux-schnell' | 'flux-dev'
  variant_index INTEGER     NOT NULL DEFAULT 0,  -- índice de variante (0-3)
  is_selected   BOOLEAN     NOT NULL DEFAULT false,  -- variante aplicada al bloque
  metadata      JSONB       NOT NULL DEFAULT '{}'::jsonb,
  created_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Recuperar variantes de un job ordenadas
CREATE INDEX idx_ai_images_job_id
  ON public.ai_images (job_id, variant_index ASC);

-- Búsqueda por user para gestión de historial
CREATE INDEX idx_ai_images_user_id_created
  ON public.ai_images (user_id, created_at DESC);
...
CREATE POLICY "Users can update own ai_images"
  ON public.ai_images FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid());


-- ── 4. Función helper: rate_limit_check ──────────────────────
-- Devuelve TRUE si el usuario está dentro del límite (10/hora).
-- Llamada desde la Edge Function antes de iniciar generación.
...
BEGIN
  ...
  VALUES (
    'ai-images',
    'ai-images',
    true,                              -- público: las imágenes se referencian desde HTML de mailing
...
-- Política de storage: usuarios autenticados pueden subir sus propias imágenes
