...
export function buttonTemplate({
  ...
  colors,
  btnFontSize = 14,
  btnFontFamily = "Arial,Helvetica,sans-serif",
  btnBorderRadius = 6,
  btnBorderWidth = 0,
  btnBorderColor = "transparent",
}: ButtonTemplateData): string {
  const borderStyle = btnBorderWidth > 0
    ? `border:${btnBorderWidth}px solid ${btnBorderColor};`
    : "";
  const radius = `${btnBorderRadius}px`;
  return `<tr>
      ...
      <tr>
        <td valign="middle" align="${align}">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;display:inline-table;">
            <tr>
              <td valign="middle" bgcolor="${colors.primary}" style="background-color:${colors.primary};border-radius:${radius};${borderStyle}mso-padding-alt:0;width:1%;white-space:nowrap;">
                <a href="${href}"
                   target="_blank"
                   style="display:inline-block;padding:14px 28px;font-size:${btnFontSize}px;line-height:20px;font-weight:700;font-family:${btnFontFamily};color:${colors.primaryForeground};text-decoration:none;border-radius:${radius};white-space:nowrap;mso-hide:none;">
